    ...
    <body class="font-montserrat min-h-screen bg-cover bg-center bg-no-repeat bg-fixed"
        ...
        <main class="flex-1 p-4 md:p-6 space-y-6 lg:ml-72">
            ...
            <div class="relative rounded-2xl bg-white p-6 md:p-10 shadow-xl min-h-[300px] border border-gray-300">
                ...
                <div class="mt-6 flex justify-end md:absolute md:bottom-6 md:right-10 w-full z-10">

                    <form method="POST" action="{{ route('logout') }}" class="md:ml-auto">
                        @csrf
                        <button type="submit"
                            class="w-auto flex items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-[#0284FD] to-[#3207CC] px-6 py-3 text-white text-sm shadow hover:opacity-90 cursor-pointer hover:scale-[1.025] transition-all">
        ...
        <div id="modal" class="fixed inset-0 hidden items-center justify-center bg-black/60 z-[999]">
            <div class="w-[400px] rounded-2xl bg-white p-6 shadow-xl relative **:focus:outline-none">
                ...
                <h2 class="mb-6 text-center text-lg font-bold text-[#1B4597]">Ubah Profil</h2>
                ...
                <form method="POST" action="{{ route('admin.profile-ketua-jurusan.update', $user->id_user) }}">
                    ...
                    <div class="flex justify-center">
                        <button type="submit"
                            class="w-40 rounded-xl bg-gradient-to-r bg-gradient-to-r from-[#0284FD] to-[#3207CC]  py-2 text-white font-semibold cursor-pointer hover:scale-[1.025] transition-all hover:opacity-90 mt-4">
                            Simpan
                        </button>
                    </div>
                </form>
        ...
        <script src="{{ asset('js/profil_ketua-jurusan.js') }}"></script>
    </body>
